...
class AiModelFragment : Fragment() {
    ...
    private fun setupSettingsListeners() {
        ...
        // ── 换题灵敏度（悬浮球计时：区域 OCR 相似度阈值） ──
        val btnTimerSensitivity = view?.findViewById<TextView>(R.id.btn_timer_sensitivity) ?: return
        val tvTimerSensitivityDesc = view?.findViewById<TextView>(R.id.tv_timer_sensitivity_desc) ?: return
        fun updateTimerSensitivityDisplay() {
            val v = AppPreferences.getTimerSimilarityThreshold(ctx)
            tvTimerSensitivityDesc.text = "相似度 < ${(v * 100).toInt()}% 判为换题"
        }
        updateTimerSensitivityDisplay()
        btnTimerSensitivity.setOnClickListener {
            val options = arrayOf("宽松（相似度 < 35%，不易误判）", "标准（相似度 < 50%）", "灵敏（相似度 < 65%，换题更容易触发）")
            val checked = when {
                AppPreferences.getTimerSimilarityThreshold(ctx) <= 0.4f -> 0
                AppPreferences.getTimerSimilarityThreshold(ctx) >= 0.6f -> 2
                else -> 1
            }
            AlertDialog.Builder(ctx)
                .setTitle("换题灵敏度")
                .setSingleChoiceItems(options, checked) { dialog, which ->
                    val v = when (which) { 0 -> 0.35f; 2 -> 0.65f; else -> 0.5f }
                    AppPreferences.setTimerSimilarityThreshold(ctx, v)
                    updateTimerSensitivityDisplay()
                    dialog.dismiss()
                }
                .setNegativeButton("取消", null)
    ...
    private fun showBankImportDialog(uri: android.net.Uri) {
        ...
        val importDialog = AlertDialog.Builder(ctx, R.style.TransparentDialog)
            .setView(layout)
