...
class MainActivity : AppCompatActivity(), HomeFragment.ServiceControlListener {
    ...
    override fun onSaveInstanceState(outState: Bundle) {
        ...
        // 底部导航（手机）与侧边导航栏（平板）共用 id，均为 NavigationBarView 子类
        val navBar = findViewById<NavigationBarView>(R.id.bottom_nav)
        if (navBar != null) {
            outState.putInt("active_tab_id", navBar.selectedItemId)
        }
    }
    ...
    private fun setupBottomNav(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            ...
            navBar.selectedItemId = restoredSelectedId
        }
    }
