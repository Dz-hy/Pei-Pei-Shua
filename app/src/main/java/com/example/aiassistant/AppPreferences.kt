...
object AppPreferences {
    ...
    private const val KEY_SHIZHENG_MODEL = "shizheng_model"
    ...
    /** 时政模块专用 AI 模型（独立于主体 AI，存单个 AiModelConfig JSON） */
    fun getShizhengModel(context: Context): String =
        prefs(context).getString(KEY_SHIZHENG_MODEL, "") ?: ""

    fun setShizhengModel(context: Context, json: String) =
        prefs(context).edit().putString(KEY_SHIZHENG_MODEL, json).apply()
    ...
    /** 计时区域按前台 App 包名分别记忆；取不到包名时用全局 key 兜底 */
    private const val KEY_TIMER_REGION_PREFIX = "timer_region_"
    const val TIMER_REGION_GLOBAL_KEY = "_global"

    /** 换题判定：区域 OCR 文字相似度低于该值视为换题（0~1） */
    private const val KEY_TIMER_SIM_THRESHOLD = "timer_similarity_threshold"

    fun getTimerSimilarityThreshold(context: Context): Float =
        prefs(context).getFloat(KEY_TIMER_SIM_THRESHOLD, 0.5f)

    fun setTimerSimilarityThreshold(context: Context, v: Float) =
        prefs(context).edit().putFloat(KEY_TIMER_SIM_THRESHOLD, v.coerceIn(0.1f, 0.9f)).apply()

    fun getTimerRegionForApp(context: Context, appKey: String): Rect? {
        val raw = prefs(context).getString(KEY_TIMER_REGION_PREFIX + appKey, null) ?: return null
        return try {
            val parts = raw.split(",")
    ...
    fun setTimerRegionForApp(context: Context, appKey: String, rect: Rect) {
        prefs(context).edit()
            .putString(KEY_TIMER_REGION_PREFIX + appKey, "${rect.left},${rect.top},${rect.right},${rect.bottom}")
            .apply()
    }

    fun clearTimerRegions(context: Context) {
        val ed = prefs(context).edit()
        prefs(context).all.keys.filter { it.startsWith(KEY_TIMER_REGION_PREFIX) }.forEach { ed.remove(it) }
        ed.apply()
    }
    ...
    /** 做题页 AI 解析记住的上次模型（独立于全局激活模型，避免影响截屏识别） */
    private const val KEY_PRACTICE_AI_MODEL_ID = "practice_ai_model_id"

    fun getPracticeAiModelId(context: Context): String =
        prefs(context).getString(KEY_PRACTICE_AI_MODEL_ID, "") ?: ""

    fun setPracticeAiModelId(context: Context, id: String) =
        prefs(context).edit().putString(KEY_PRACTICE_AI_MODEL_ID, id).apply()
    ...
    // 默认菜单项：词典, 截图模式, 计时, 关闭
    private const val DEFAULT_MENU_ITEMS = "dict,capture_mode,timer,close"
    private const val KEY_MENU_ITEMS_TIMER_ADDED = "ball_menu_items_timer_added"

    fun getBallMenuItems(context: Context): List<String> {
        val p = prefs(context)
        val raw = p.getString(KEY_MENU_ITEMS, DEFAULT_MENU_ITEMS) ?: DEFAULT_MENU_ITEMS
        var list = raw.split(",").filter { it.isNotBlank() }
        // 一次性迁移：给升级上来的老用户自动补上新增的计时菜单项
        if (!p.getBoolean(KEY_MENU_ITEMS_TIMER_ADDED, false)) {
            val extra = listOf("timer", "timer_history").filter { it !in list }
            if (extra.isNotEmpty()) list = list + extra
            p.edit()
                .putString(KEY_MENU_ITEMS, list.joinToString(","))
                .putBoolean(KEY_MENU_ITEMS_TIMER_ADDED, true)
                .apply()
        }
        return list
    }
    ...
    const val CLICK_ACTION_AI_ANALYZE = 0   // AI 搜题/分析
    const val CLICK_ACTION_RECORD_WRONG = 1  // 记录错题（默认）

    fun getFloatClickAction(context: Context): Int =
        prefs(context).getInt(KEY_FLOAT_CLICK_ACTION, CLICK_ACTION_RECORD_WRONG)
