...
// ── 回到本 App 时收起 / 离开时恢复（App 生命周期驱动） ──────────────

internal fun ScreenCaptureService.hideBallForInApp() {
    mainHandler.post {
        dismissBallMenu()
        detachFloatBall()
        detachSmallBall()
...
internal fun ScreenCaptureService.restoreBallAfterInApp() {
    mainHandler.post {
        reattachFloatBall()
...
internal fun ScreenCaptureService.showBallMenu() {
    ...
    val timerItem = menuView.findViewById<TextView>(R.id.menu_timer)
    val timerHistoryItem = menuView.findViewById<View>(R.id.menu_timer_history)
    ...
    val divider4 = menuView.findViewById<View>(R.id.menu_divider_4)
    val dividerTimer = menuView.findViewById<View>(R.id.menu_divider_timer)
    ...
    timerItem?.visibility = if (enabledItems.contains("timer")) View.VISIBLE else View.GONE
    timerHistoryItem?.visibility = if (enabledItems.contains("timer_history")) View.VISIBLE else View.GONE

    // 计时项动态文案
    timerItem?.text = when {
        TimerEngine.isRunning || TimerEngine.isCountingDown -> "停止计时"
        else -> "开始计时"
    }

    // 智能连贯的分隔符控制
    val hasTimer = timerItem?.visibility == View.VISIBLE
    val hasTimerHistory = timerHistoryItem?.visibility == View.VISIBLE
    ...
    dividerTimer.visibility = if (hasTimer && hasTimerHistory) View.VISIBLE else View.GONE
    // divider4 在计时记录与关闭悬浮球之间
    divider4.visibility = if (closeItem.visibility == View.VISIBLE && (hasTimer || hasTimerHistory)) View.VISIBLE else View.GONE
    ...
    timerItem?.setOnClickListener {
        dismissBallMenu()
        TimerEngine.onTimerMenuClicked(this)
    }
    // 长按计时项：重新框选当前 App 的计时区域
    timerItem?.setOnLongClickListener {
        dismissBallMenu()
        TimerEngine.reselectRegion(this)
        true
    }

    timerHistoryItem?.setOnClickListener {
        dismissBallMenu()
        openTimerHistory()
    }
...
private fun ScreenCaptureService.applyBallMenuSize(menuView: View) {
    ...
    val itemIds = listOf(R.id.menu_dict, R.id.menu_capture_mode, R.id.menu_click_action, R.id.menu_switch_teacher, R.id.menu_timer, R.id.menu_timer_history, R.id.menu_close_ball)
    for (id in itemIds) {
