...
object QuestionMatcher {
    ...
    fun match(context: Context, ocrText: String, onResult: (MatchResult) -> Unit) {
        val appCtx = context.applicationContext
        Thread {
            onResult(matchBlocking(appCtx, ocrText))
        }.start()
    }

    fun matchBlocking(context: Context, ocrText: String): MatchResult {
        ...
        try {
            QuestionBankManager.search(cleaned)?.let {
                return MatchResult(it, CONF_AUTO, listOf(it), 1f)
            }
        } catch (e: Exception) {
            e.printStackTrace()
        }
        ...
        // ② 向量召回（未配置向量服务则到此为止）
        if (!AppPreferences.hasEmbConfig(context)) return MatchResult(null, CONF_NONE)
        ...
        return try {
            ...
            val sims = db.loadAllVectors()
                ...
                .take(VECTOR_TOP_K)
            if (sims.isEmpty()) return MatchResult(null, CONF_NONE)

            val candidates = sims.mapNotNull { db.getQuestionById(it.first) }
            if (candidates.isEmpty()) return MatchResult(null, CONF_NONE)
            val bestSim = sims.first().second

            // 相似度直接过自动阈值：不再耗 LLM
            if (bestSim >= AppPreferences.getMatchVectorAutoThreshold(context)) {
                return MatchResult(candidates.first(), CONF_AUTO, candidates, bestSim)
            }

            // ③ LLM 裁决
            val verdict = rerankWithLlm(context, cleaned, candidates)
            if (verdict != null && verdict.second == "high" && verdict.first in candidates.indices) {
                MatchResult(candidates[verdict.first], CONF_AUTO, candidates, bestSim)
            } else {
                MatchResult(null, CONF_CONFIRM, candidates, bestSim)
            }
        } catch (e: Exception) {
            e.printStackTrace()
    ...
    private fun rerankWithLlm(context: Context, ocrText: String, candidates: List<Question>): Pair<Int, String>? {
        val userMessage = buildString {
            ...
            appendLine()
            append("判断 OCR 文本对应的原题是候选中的第几道；若都不匹配则 match 为 null。只输出 JSON：{\"match\": <编号或null>, \"confidence\": \"high\"|\"medium\"|\"low\"}")
        }
        ...
        for (model in buildModelChain(context)) {
            ...
            try {
                OpenAIApiService.analyzeText(
                    ...
                    apiKey = model.apiKey,
                    model = model.model,
                    prompt = "你是题目匹配裁判。OCR 文本与题库原题可能有轻微字词差异，但考点、选项设置、题干结构一致的是同一题。严格区分相似但不同的题（数字、设问方向不同即为不同题）。",
                    thinking = false,
