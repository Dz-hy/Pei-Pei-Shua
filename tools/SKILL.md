...
**判断题识别**：`【解析N—正确答案√】`/`故表述正确/错误` 都会被判判断题跳过；多选（答案 ABD/BD）自动跳过。广东卷判断/多选集中在第一大题，预期跳过 6-10 题。

**2026 卷现状**：题干+材料完整（`26广东省考试《行测》真题及答案（完整版）.pdf`，仅图形推理 46-50、科学推理 67-70 共 9 处内嵌答案），**答案源缺**——爱真题下载 3.9 元、道客巴巴要积分、网盘分享被ban，全部受阻。拿到答案源后走场景 C 的 `--answer` 或配解析册转换。
